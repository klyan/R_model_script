...
fit <- glmnet(train, as.factor(train.label),family="binomial", alpha = 1,weights = data_sample_weight)

save.image(file="./id_modelv3_addfeature.RData") 

trainY <- predict(fit,train,s=0.0010410,type="response")
testY <- predict(fit,test,s=0.0010410,type="response")

colAUC(trainY, csvdata[train_index,"label"])
colAUC(testY, csvdata[-train_index,"label"])

colAUC(trainY, csvdata[train_index,"real_order"])
colAUC(testY, csvdata[-train_index,"real_order"])

coeff <- coef(fit,s=0.0010410)
...
wf <- file("/data/kai.zhang/model_merge/FoodCat1IdLRSeq3.json", "w" )
